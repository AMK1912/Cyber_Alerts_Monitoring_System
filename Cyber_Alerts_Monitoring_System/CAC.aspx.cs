...
namespace CyberAlert
{
    public partial class CyberAlertsCentral : Page
    {
        ...
        protected void btnSubmitAlert_Click(object sender, EventArgs e)
        {
            ...
            System.Diagnostics.Debug.WriteLine("CAC.aspx.cs btnSubmitAlert_Click - Session[Center]: " + (Session["Center"] != null ? Session["Center"].ToString() : "NULL"));
            ...
            if (Session["Center"] != null && Session["Center"].ToString() == "CO")
            {
                ...
                if (string.IsNullOrEmpty(alertDateTime) || string.IsNullOrEmpty(receivedDate)||
                    ...
                    string.IsNullOrEmpty(repliedSender)|| string.IsNullOrEmpty(closingDate)) // Removed duplicate alertDetails
                {
                ...
                DateTime actionDt, repliedSenderDt, closingDt; // Declare for parsing
                ...
                try
                {
                    using (OleDbConnection currentConn = new OleDbConnection(ConfigurationManager.ConnectionStrings["constring"].ToString())) // Use a local connection object for the submit
                    {
                        currentConn.Open();
                        string procedureName = "CYBER_ALERT_SUBMISSION";
                        OleDbCommand cmd = new OleDbCommand(procedureName, currentConn); // Use currentConn
                        ...
                        cmd.Parameters.Add("vFIRST_ACTION_DATE", OleDbType.Date).Value = actionDt; // Use parsed date
                        ...
                        cmd.Parameters.Add("vREPLIED_SENDER_DATE", OleDbType.Date).Value = repliedSenderDt; // Use parsed date
                        cmd.Parameters.Add("vCLOSING_DATE", OleDbType.Date).Value = closingDt; // Use parsed date
                        cmd.ExecuteNonQuery();
                ...
                // No finally block needed if using 'using' statement for connection
            }
            else
            {
